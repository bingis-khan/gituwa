...
type repo *C.git_repository

func repository_open(path string) (repo, error) {
...
func list_all(repo *C.git_repository) ([]string, error) {
	...
	var names []string
	error = C.git_tree_walk(tree, C.GIT_TREEWALK_PRE, (*[0]byte)(C.list_cb), unsafe.Pointer(&names))  // note, that we're getting our Go function *through* C
	if error < 0 {
		return nil, to_error(error)
	}
...
func list_cb(root *C.char, entry *C.git_tree_entry, payload *C.void) {
	...
	names := (*[]string)(unsafe.Pointer(payload))
